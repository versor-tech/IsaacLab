...
"""
Franka arm reaching to a cube with Viser visualization.
...
while providing interactive 3D visualization via Viser.

Usage:
    ./isaaclab.sh -p scripts/demos/franka_reach_cube_viser.py --headless --enable_cameras --num_envs 1
...
parser = argparse.ArgumentParser(description="Franka arm reaching to cube with Viser visualization.")
...
class FrankaReachCubeEnv(DirectRLEnv):
    """Franka arm reaching to cube environment with Viser visualization."""
...
def main():
    """Main function."""
    # Start Viser server
    print(f"[INFO]: Starting Viser server on port {args_cli.port}...")
    server = viser.ViserServer(port=args_cli.port)
    print(f"[INFO]: Viser server started. Open http://localhost:{args_cli.port} in your browser.")
    ...
    # Create environment with rgb_array render mode for Viser
    ...
    with torch.inference_mode():
        while True:
            try:
                # Step the environment (actions are not used, IK computes internally)
                actions = torch.zeros(
                    (args_cli.num_envs, env_cfg.action_space), device=env.device, dtype=torch.float32
                )
                env.step(actions)

                # Get connected Viser clients
                clients = server.get_clients()
                if not clients:
                    # If no client connected, just step simulation
                    time.sleep(0.01)
                    continue

                # Render frame for Viser
                frame = env.render()
                ...
                if frame is not None:
                    ...
                    # Update Viser background with our rendered frame
                    server.scene.set_background_image(frame, format="jpeg", jpeg_quality=80)
